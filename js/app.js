...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `
      <div class="rounded-3 bg-white p-3 m-3">
        <div>
          <img src="${product.image}" width="150px" height="150px"></img>
        </div>
        <div class="text-start">
          <h5 class="mt-3">${product.title}</h5>
          <p class="text-secondary">${product.category}</p>
          <h2><strong>$${product.price}</strong></h2>
          <p class="text-primary"><i class="fas fa-star"></i> ${product.rating.rate} (${product.rating.count} total)</p>
          <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-primary w-100 mb-2">add to cart</button>
          <button id="details-btn" class="btn btn-danger w-100">Details</button>
        </div>
      </div>
      `;
